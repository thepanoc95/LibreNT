...
## Deliverables (.sys + .dll)

- `posixsubsystem.dll` (user-mode runtime/translation layer)
- `posixsubsys.sys` (WDK-built kernel companion using supported device/IOCTL interfaces)
- shared ABI header `posix_abi.h` used by both layers

This keeps POSIX support usable while avoiding kernel image patching.
